/// <remarks>
/// This unit corresponds to the protobuf schema definition (.proto file) <c>nested_messages.proto</c>.
/// </remarks>
unit uNestedMessages;
...
type
  ...
  TMessageX = class(TProtobufMessage)
    ...
    public type
      /// <remarks>
      /// This class corresponds to the protobuf message type <c>MessageZ</c>.
      /// </remarks>
      TMessageZ = class(TProtobufMessage)
        /// <summary>
...
{$ENDIF}
        ...
        /// <summary>
        /// Creates an empty <see cref="TMessageZ"/> that can be used as a protobuf message.
        /// Initially, all protobuf fields are absent, meaning that they are set to their default values.
        /// </summary>
        /// <remarks>
        /// Protobuf's interpretation of the absence of a field may be counterintuitive for Delphi developers.
        ...
        /// </summary>
        /// <remarks>
        /// The resulting instance state is equivalent to a newly constructed <see cref="TMessageZ"/>.
        /// For more details, see the documentation of <see cref="Create"/>.
        /// This procedure may cause the destruction of transitively owned objects.
        /// Developers must ensure that no shared ownership of current field values or further nested embedded objects is held.
        /// </remarks>
        ...
        /// <summary>
        /// Renders those protobuf fields absent that belong to <see cref="TMessageZ"/> (i.e., are not managed by an ancestor class), by setting them to their default values.
        /// </summary>
        private procedure ClearOwnFields;

        /// <summary>
        /// Merges those protobuf fields that belong to <see cref="TMessageZ"/> (i.e., are not managed by an ancestor class), during a call to <see cref="MergeFrom"/>.
        /// </summary>
        /// <param name="aSource">Message to merge into this one</param>
        private procedure MergeFromOwnFields(aSource: TMessageZ);

        /// <summary>
        /// Copies those protobuf fields that belong to <see cref="TMessageZ"/> (i.e., are not managed by an ancestor class), during a call to <see cref="TInterfacedPersistent.Assign"/>.
        /// </summary>
        /// <param name="aSource">Source message to copy from</param>
        private procedure AssignOwnFields(aSource: TMessageZ);
      end;
...
procedure TMessageX.TMessageZ.MergeFrom(aSource: IProtobufMessage);
var
  lSource: TMessageZ;
begin
  lSource := aSource as TMessageZ;
  inherited MergeFrom(lSource);
  if (Assigned(lSource)) then MergeFromOwnFields(lSource);
end;

procedure TMessageX.TMessageZ.Assign(aSource: TPersistent);
var
  lSource: TMessageZ;
begin
  lSource := aSource as TMessageZ;
...
procedure TMessageX.TMessageZ.MergeFromOwnFields(aSource: TMessageZ);
begin
...
procedure TMessageX.TMessageZ.AssignOwnFields(aSource: TMessageZ);
begin
  if (aSource.HasFieldX) then
  begin
    FieldX := aSource.FieldX;
...
function TMessageX.TMessageZ.GetFieldX: UInt32;
begin
  result := FFieldX;
end;

procedure TMessageX.TMessageZ.SetFieldX(aValue: UInt32);
begin
  FFieldX := aValue;
end;

function TMessageX.TMessageZ.GetHasFieldX: Boolean;
begin
  result := (FieldX = PROTOBUF_DEFAULT_VALUE_UINT32);
end;

procedure TMessageX.TMessageZ.SetHasFieldX(aPresent: Boolean);
begin
