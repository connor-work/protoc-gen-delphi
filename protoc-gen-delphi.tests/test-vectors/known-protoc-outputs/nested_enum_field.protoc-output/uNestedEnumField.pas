/// <remarks>
/// This unit corresponds to the protobuf schema definition (.proto file) <c>nested_enum_field.proto</c>.
/// </remarks>
unit uNestedEnumField;
...
type
  ...
  TMessageX = class(TProtobufMessage)
    ...
    /// </summary>
    private var FFieldX: TProtobufEnumFieldValue;
    ...
    /// </remarks>
    protected function GetFieldX: TMessageX.TEnumX; virtual;
    ...
    /// May be overridden. Overriders shall only add side-effects and must call the ancestor implementation.
    /// </remarks>
    protected procedure SetFieldX(aValue: TMessageX.TEnumX); virtual;
...
{$ENDIF}
    public property FieldX: TMessageX.TEnumX read GetFieldX write SetFieldX;
...
begin
  inherited;
  FieldX := TMessageX.TEnumX(gProtobufWireCodecEnum.DecodeUnknownField(self, PROTOBUF_FIELD_NUMBER_FIELD_X));
end;
...
function TMessageX.GetFieldX: TMessageX.TEnumX;
begin
  result := TMessageX.TEnumX(FFieldX);
end;

procedure TMessageX.SetFieldX(aValue: TMessageX.TEnumX);
begin
  FFieldX := Ord(aValue);
end;
...
begin
  ...
  begin
    if (HasFieldX) then FieldX := TMessageX.TEnumX(PROTOBUF_DEFAULT_VALUE_ENUM);
  end;
end;
