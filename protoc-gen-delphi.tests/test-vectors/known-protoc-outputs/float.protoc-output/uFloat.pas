...
begin
  inherited;
  gProtobufWireCodecFloat.EncodeSingularField(FFieldX, self, PROTOBUF_FIELD_NUMBER_FIELD_X, aDest);
end;

procedure TMessageX.Decode(aSource: TStream);
begin
  inherited;
  FFieldX := gProtobufWireCodecFloat.DecodeUnknownField(self, PROTOBUF_FIELD_NUMBER_FIELD_X);
end;
...
begin
  if (aSource.FieldX <> PROTOBUF_DEFAULT_VALUE_FLOAT) then FieldX := aSource.FieldX;
end;
