...
begin
  inherited;
  gProtobufWireCodecBool.EncodeSingularField(FFieldX, self, PROTOBUF_FIELD_NUMBER_FIELD_X, aDest);
end;

procedure TMessageX.Decode(aSource: TStream);
begin
  inherited;
  FFieldX := gProtobufWireCodecBool.DecodeUnknownField(self, PROTOBUF_FIELD_NUMBER_FIELD_X);
end;
...
begin
  if (aSource.FieldX <> PROTOBUF_DEFAULT_VALUE_BOOL) then FieldX := aSource.FieldX;
end;
