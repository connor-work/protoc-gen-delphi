...
{$ENDIF}
    ...
    /// If present, setting it to absent sets it to its default value <see cref="PROTOBUF_DEFAULT_VALUE_BOOL"/>.
    ...
    /// This means that it is considered present when its value does not equal the default value <see cref="PROTOBUF_DEFAULT_VALUE_BOOL"/>.
    /// </remarks>
...
begin
  inherited;
  gProtobufWireCodecBool.EncodeSingularField(FieldX, self, PROTOBUF_FIELD_NUMBER_FIELD_X, aDest);
end;

procedure TMessageX.Decode(aSource: TStream);
begin
  inherited;
  FieldX := gProtobufWireCodecBool.DecodeUnknownField(self, PROTOBUF_FIELD_NUMBER_FIELD_X);
end;
...
begin
  result := (FieldX = PROTOBUF_DEFAULT_VALUE_BOOL);
end;
...
begin
  ...
  begin
    if (HasFieldX) then FieldX := PROTOBUF_DEFAULT_VALUE_BOOL;
  end;
end;
